...
import {getArticleById, comment} from "@/api/article.ts"
...
const likeArticle = _.throttle(async () => {
    ...
    article.value.articleDetailInfo.isLiked = res.data
            ...
            <div class="comment mt-[15px] bg-white">
                ...
                <div class="comment-area">
                    <div
                        ...
                        <span class="comment-count">共 9999+ 条评论</span>
                    </div>
                    <div class="comment-area-main" v-show-viewport="() => console.log(123)">
                        <el-empty v-if="!article.commentDetailInfoVO?.length"
                                  description="暂无评论，快来抢占沙发吧！"></el-empty>
                        <div v-else class="comments">
                            <div
                                class="comment border-b-[1px] py-[10px] px-[15px]"
                                v-for="comment in article.commentDetailInfoVO"
                                :key="comment.id"
                            >
                                <div class="topper h-[55px] flex items-center justify-between">
                                ...
                                <div class="comment-content w-[70%] max-h-[150px] h-auto ml-[50px]">
                                    {{ comment.content }}
                                </div>
                                <div class="tools h-[25px] flex justify-end text-lg items-center">
                                    <i class="iconfont icon-like-comment mr-[25px] hover:text-black cursor-pointer"></i>
                                    <i class="iconfont icon-dislike-comment mr-[25px] hover:text-black cursor-pointer"></i>
                                    <i class="iconfont icon-comment mr-[25px] hover:text-black cursor-pointer"></i>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        <aside
            ...
            <div class="author w-full flex-[0.36] bg-white flex flex-col">
                ...
                >
                    ...
                    <div class="main-author flex flex-col w-[60px] items-center z-[1] mt-[-30px] cursor-pointer" @click="$router.push({ name: 'profile', params: { userId: article.authorDetailInfo?.id } })">
                        <div class="avatar-frame w-[70px] h-[70px] p-[3px] bg-white rounded-full">
                            <img :src="article.authorDetailInfo?.avatar"
                                 class="w-full h-full rounded-full object-cover"/>
                        </div>
                        <span class="font-serif text-lg mt-[5px]">{{ article.authorDetailInfo?.username }}</span>
                    </div>
                    <div
                        class="author-info w-full h-full absolute top-0 left-0 bg-gradient-to-t from-white from-40% px-[10px] flex items-end"
                    >
                        <div
                            class="container w-full h-[100px] flex text-sm p-[15px] flex-wrap justify-between">
                            <div class="gender ">
                ...
                <div class="container w-full flex-1">
                    ...
                    <div class="links w-full h-full flex flex-col p-[10px]">
                        <div
                            ...
                            :key="item.id"
                            title="点击查看文章详情"
                            @click="$router.push(`/xingz-cm/article/${item.id}`)"
                        >
                            <div class="title text-sm w-full flex justify-between">
                                <span>{{ item.title }}</span>
                                <span v-if="article.articleDetailInfo?.id === item.id" class="text-[#ccc]">本文</span>
